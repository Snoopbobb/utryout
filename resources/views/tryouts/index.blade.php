	...
	@if( count($tryouts) > 0 )
		...
		@foreach ($tryouts as $tryout)
			<div class="tryout .col-xs-6 .col-md-4">
				...
				<a href="{{ url('/tryouts/') . '/' . $tryout->sport . '/' . strtolower($tryout->state) . '/' . seoUrl(strtolower($tryout->city)) . '/'  .   $tryout->id . '/' . seoUrl(strtolower($tryout->organization))}}" class="btn">Additional Information</a>
			</div>
		@endforeach
	@else
			@if(!empty($sport))
